...
def explode():
    ...
    try:
        ...
        explode()
def qlist(series,returntype,asking = "What would you like to do? "):
    #Thanks to Ian, I have made this. I also learned pass from his original block of code

    if "Back" in series:
        for option in range(len(series)):
            q(f"{option} - {series[option]}\n")
            wait()
        option = ask(asking)
        if returntype:
            try:
        ...
        for option in range(len(series)-1):
            q(f"{option+1} - {series[option+1]}\n")
            wait()
        option = ask(asking)
        if returntype:
            try:
